...
from utils.data_loader_cfde import (
    load_author_nodes,
...
status = st.empty()
show_bootstrap_ui = not st.session_state.get("cfde_bootstrap_done", False)
...
if faiss_index is None or not author_ids:
    st.error("Search index not available. Please provide local data under `cfde_demo/data`. See README.")
    st.stop()
...
status.empty()
if show_bootstrap_ui:
    st.session_state["cfde_bootstrap_done"] = True
...
if "cfde_warmup_done" not in st.session_state:
    with st.spinner("Warming up resources…"):
        ...
        try:
            ...
            pass
    st.session_state["cfde_warmup_done"] = True
